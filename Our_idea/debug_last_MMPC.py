...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 17 16:00:02 2023
...
def simu(Patient_info: list, style: str, MPC_param: list, EKF_param: list,
    ...
    for i in range(model_number):
        ...
        Controller_list.append(MPC_lin(A_nom, B_nom,
    ...
    MMPC = Euristic_MMPC(Estimator_list, Controller_list, hysteresis=2, window_length=10, best_init=13)
    if style == 'induction':
        N_simu = int(10 / ts) * 60
        BIS = np.zeros(N_simu)
        ...
        for i in range(N_simu):
            ...
            uP, uR = Controller_list[6].one_step(X_real, BIS_cible, BIS[i])
...
Patient_table = [[1,  40, 163, 54, 0, 4.73, 24.97,  1.08,  0.30, 97.86, 89.62],
                 ...
                 [14, 38, 169, 65, 0, 2.46,	26.00,	2.53,  0.00,  97.4,	97.4]]
...
MPC_param = [30, 30, 10**(2)*np.diag([10, 1]), 0.004]
EKF_param = [1, -1, 1]
phase = 'induction'
ts = 1
