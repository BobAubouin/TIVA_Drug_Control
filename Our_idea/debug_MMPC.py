...
from TIVA_Drug_Control.Our_idea.MMPC_control import Bayes_MMPC_best, Bayes_MMPC_mean, Euristic_MMPC, Euristic_Mirko
...
def simu(Patient_info: list, style: str, MPC_param: list, EKF_param: list,
    ...
    # MMPC = Euristic_Mirko(Estimator_list, Controller_list, BIS_parameters, A_nom, B_nom,
    # hysteresis = 3, window_length = 20, best_init = 13)
    if style == 'induction':
        N_simu = int(10 / ts) * 60
        BIS = np.zeros(N_simu)
...
# MPC_param = [30, 30, 10**(1.88) * np.diag([3, 1]), 4e-2]
# param_opti = pd.read_csv('optimal_parameters_MPC_lin.csv')
# param_opti = [int(param_opti['N']), int(param_opti['Nu']), float(param_opti['R']), float(param_opti['ki'])]
# MPC_param = [30, 30, 10**param_opti[2]*np.diag([3, 1]), 10**param_opti[3]]
